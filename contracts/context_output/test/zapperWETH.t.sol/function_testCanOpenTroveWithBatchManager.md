...
function testCanOpenTroveWithBatchManager() external {
    ...
    IZapper.OpenTroveParams memory params = IZapper.OpenTroveParams({owner: A, ownerIndex: 0, collAmount: 0, boldAmount: boldAmount, upperHint: 0, lowerHint: 0, annualInterestRate: 0, batchManager: B, maxUpfrontFee: 1000e18, addManager: address(0), removeManager: address(0), receiver: address(0)});
    vm.startPrank(A);
...
```
┌─ [0] ⚙️ FUNCTION: ZapperWETHTest.testCanOpenTroveWithBatchManager() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
