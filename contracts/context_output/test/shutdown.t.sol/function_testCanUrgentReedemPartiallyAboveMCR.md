# Function: testCanUrgentReedemPartiallyAboveMCR()
...
function testCanUrgentReedemPartiallyAboveMCR() external {
    ...
    assertEq(contractsArray[0].collToken.balanceOf(A), collBalanceBefore + ((((redemptionAmount * DECIMAL_PRECISION) / price) * (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS)) / DECIMAL_PRECISION), "Coll balance mismatch");
}
```
...
```
┌─ [0] ⚙️ FUNCTION: ShutdownTest.testCanUrgentReedemPartiallyAboveMCR() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │   💬 Args: [icr, MCR, "Trove CR should be above MCR"]
  │   👁️  Def: internal
  ...
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 13)
      💬 Args: [contractsArray[0].collToken.balanceOf(A), collBalanceBefore + ((((redemptionAmount * DECIMAL_PRECISION) / price) * (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS)) / DECIMAL_PRECISION), "Coll balance mismatch"]
      👁️  Def: internal
```
