# Function: testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerFLProvider()
...
function testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerFLProvider() external {
    for (uint256 i = 0; i < 3; i++) {
        _testOnlyOwnerOrManagerCanLeverDownFromBalancerFLProvider(leverageZapperHybridArray[i], ExchangeType.HybridCurveUniV3, i);
    }
}
```
...
### _testOnlyOwnerOrManagerCanLeverDownFromBalancerFLProvider(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256)

- **Kind**: internal
- **Source**: 58541:2084:338
- **Link**: `test/zapperLeverage.t.sol:ZapperLeverageMainnet:_testOnlyOwnerOrManagerCanLeverDownFromBalancerFLProvider(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256)`

```solidity
function _testOnlyOwnerOrManagerCanLeverDownFromBalancerFLProvider(ILeverageZapper _leverageZapper, ExchangeType _exchangeType, uint256 _branch) internal {
    ...
    vm.startPrank(B);
    vm.expectRevert();
    flashLoanProvider.makeFlashLoan(contractsArray[_branch].collToken, flashLoanAmount, IFlashLoanProvider.Operation.LeverDownTrove, abi.encode(params));
    vm.stopPrank();
...
```
┌─ [0] ⚙️ FUNCTION: ZapperLeverageMainnet.testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerFLProvider() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: ZapperLeverageMainnet._testOnlyOwnerOrManagerCanLeverDownFromBalancerFLProvider(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256) (NodeID: 1)
    ...
    │     👁️  Def: internal
    └─ [2] ⚙️ FUNCTION: StdAssertions.assertEq(address,address,string) (NodeID: 13)
