# Function: testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerVault()
...
function testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerVault() external {
    for (uint256 i = 0; i < 3; i++) {
        _testOnlyOwnerOrManagerCanLeverDownFromBalancerVault(leverageZapperHybridArray[i], ExchangeType.HybridCurveUniV3, i);
    }
}
```
...
### _testOnlyOwnerOrManagerCanLeverDownFromBalancerVault(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256)

- **Kind**: internal
- **Source**: 61535:2350:338
- **Link**: `test/zapperLeverage.t.sol:ZapperLeverageMainnet:_testOnlyOwnerOrManagerCanLeverDownFromBalancerVault(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256)`

```solidity
function _testOnlyOwnerOrManagerCanLeverDownFromBalancerVault(ILeverageZapper _leverageZapper, ExchangeType _exchangeType, uint256 _branch) internal {
    ...
    bytes memory userData = abi.encode(address(_leverageZapper), IFlashLoanProvider.Operation.LeverDownTrove, params);
...
```
┌─ [0] ⚙️ FUNCTION: ZapperLeverageMainnet.testOnlyOwnerOrManagerCanLeverDownWithHybridFromBalancerVault() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: ZapperLeverageMainnet._testOnlyOwnerOrManagerCanLeverDownFromBalancerVault(contract ILeverageZapper,enum ZapperLeverageMainnet.ExchangeType,uint256) (NodeID: 1)
