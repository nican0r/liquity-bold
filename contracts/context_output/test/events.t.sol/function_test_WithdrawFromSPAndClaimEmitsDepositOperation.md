# Function: test_WithdrawFromSPAndClaimEmitsDepositOperation()
...
function test_WithdrawFromSPAndClaimEmitsDepositOperation() external {
    ...
    vm.expectEmit();
    emit DepositOperation(A, Operation.withdrawFromSP, deposit.pendingBoldLoss, -int256(withdrawal), deposit.pendingBoldYieldGain, deposit.pendingBoldYieldGain, deposit.pendingCollGain, deposit.pendingCollGain + deposit.stashedColl);
...
```
┌─ [0] ⚙️ FUNCTION: StabilityPoolEventsTest.test_WithdrawFromSPAndClaimEmitsDepositOperation() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │     👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: BaseTest.makeSPWithdrawalAndClaim(address,uint256) (NodeID: 40)
      💬 Args: [A, withdrawal]
      👁️  Def: public
```
