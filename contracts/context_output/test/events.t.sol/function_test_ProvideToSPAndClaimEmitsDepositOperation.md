# Function: test_ProvideToSPAndClaimEmitsDepositOperation()
...
function test_ProvideToSPAndClaimEmitsDepositOperation() external {
    ...
    vm.expectEmit();
    emit DepositOperation(A, Operation.provideToSP, deposit.pendingBoldLoss, int256(topUp), deposit.pendingBoldYieldGain, deposit.pendingBoldYieldGain, deposit.pendingCollGain, deposit.pendingCollGain + deposit.stashedColl);
...
```
┌─ [0] ⚙️ FUNCTION: StabilityPoolEventsTest.test_ProvideToSPAndClaimEmitsDepositOperation() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │     👁️  Def: internal
  └─ [1] ⚙️ FUNCTION: BaseTest.makeSPDepositAndClaim(address,uint256) (NodeID: 40)
      💬 Args: [A, topUp]
      👁️  Def: public
```
