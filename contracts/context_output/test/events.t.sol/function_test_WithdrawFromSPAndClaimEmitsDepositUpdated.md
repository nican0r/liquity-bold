# Function: test_WithdrawFromSPAndClaimEmitsDepositUpdated()
...
function test_WithdrawFromSPAndClaimEmitsDepositUpdated() external {
    ...
    vm.expectEmit();
    emit DepositUpdated(A, (deposit.recordedBold - deposit.pendingBoldLoss) - withdrawal, 0, current.P, current.S, current.B, current.scale);
...
```
┌─ [0] ⚙️ FUNCTION: StabilityPoolEventsTest.test_WithdrawFromSPAndClaimEmitsDepositUpdated() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
