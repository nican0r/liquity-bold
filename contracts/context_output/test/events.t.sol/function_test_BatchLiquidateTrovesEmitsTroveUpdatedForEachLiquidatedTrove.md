# Function: test_BatchLiquidateTrovesEmitsTroveUpdatedForEachLiquidatedTrove()
...
function test_BatchLiquidateTrovesEmitsTroveUpdatedForEachLiquidatedTrove() external {
    ...
    vm.recordLogs();
    for (uint256 i = 0; i < liquidateTroveIds.length; ++i) {
        emit TroveUpdated(liquidateTroveIds[i], 0, 0, 0, 0, 0, 0);
    }
...
```
┌─ [0] ⚙️ FUNCTION: TroveEventsTest.test_BatchLiquidateTrovesEmitsTroveUpdatedForEachLiquidatedTrove() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
