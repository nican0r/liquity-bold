# Function: test_ProvideToSPAndClaimEmitsDepositUpdated()
...
function test_ProvideToSPAndClaimEmitsDepositUpdated() external {
    ...
    vm.expectEmit();
    emit DepositUpdated(A, (deposit.recordedBold - deposit.pendingBoldLoss) + topUp, 0, current.P, current.S, current.B, current.scale);
...
```
┌─ [0] ⚙️ FUNCTION: StabilityPoolEventsTest.test_ProvideToSPAndClaimEmitsDepositUpdated() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
