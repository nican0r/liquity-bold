# Function: testCanSetRemoveManagerOnOpenTrove()
...
function testCanSetRemoveManagerOnOpenTrove() public {
    vm.startPrank(A);
    uint256 ATroveId = borrowerOperations.openTrove(A, 0, 100 ether, 10000e18, 0, 0, 5e16, 10000e18, address(0), B, A);
    vm.stopPrank();
...
```
┌─ [0] ⚙️ FUNCTION: BorrowerOperationsOnBehalfTroveManagamentTest.testCanSetRemoveManagerOnOpenTrove() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(address,address,string) (NodeID: 2)
      💬 Args: [receiver, A, "Wrong Receiver"]
      👁️  Def: internal
```
