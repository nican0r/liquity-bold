# Function: test_OpenTroveEmitsTroveOperation()
...
function test_OpenTroveEmitsTroveOperation() external {
    ...
    vm.expectEmit();
    emit TroveOperation(troveId, Operation.openTrove, interestRate, 0, upfrontFee, int256(borrow), 0, int256(coll));
    vm.prank(owner);
...
```
┌─ [0] ⚙️ FUNCTION: TroveEventsTest.test_OpenTroveEmitsTroveOperation() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
