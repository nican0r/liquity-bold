# Function: test_RedeemCollateralEmitsTroveOperationForEachRedeemedTrove()
...
function test_RedeemCollateralEmitsTroveOperationForEachRedeemedTrove() external {
    ...
    for (uint256 i = 0; i < 3; ++i) {
        ...
        emit TroveOperation(troveId[i], Operation.redeemCollateral, interestRate, 0, 0, -int256(redeemBold[i]), 0, -int256(redeemEth - redeemFee));
    }
...
```
┌─ [0] ⚙️ FUNCTION: TroveEventsTest.test_RedeemCollateralEmitsTroveOperationForEachRedeemedTrove() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
