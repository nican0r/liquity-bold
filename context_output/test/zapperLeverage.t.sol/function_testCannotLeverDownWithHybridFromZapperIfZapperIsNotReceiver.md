# Function: testCannotLeverDownWithHybridFromZapperIfZapperIsNotReceiver()
...
function testCannotLeverDownWithHybridFromZapperIfZapperIsNotReceiver() external {
    for (uint256 i = 0; i < 3; i++) {
        _testCannotLeverDownFromZapperIfZapperIsNotReceiver(leverageZapperUniV3Array[i], ExchangeType.HybridCurveUniV3, i);
    }
}
```
...
```
┌─ [0] ⚙️ FUNCTION: ZapperLeverageMainnet.testCannotLeverDownWithHybridFromZapperIfZapperIsNotReceiver() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
