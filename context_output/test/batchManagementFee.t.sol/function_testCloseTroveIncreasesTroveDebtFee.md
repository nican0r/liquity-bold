# Function: testCloseTroveIncreasesTroveDebtFee()
...
function testCloseTroveIncreasesTroveDebtFee() public {
    ...
    assertEq(AIntialBalance - boldToken.balanceOf(A), (troveInitialDebt + troveAccruedInterest) + troveAccruedManagementFee);
}
```
...
```
┌─ [0] ⚙️ FUNCTION: BatchManagementFeeTest.testCloseTroveIncreasesTroveDebtFee() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256) (NodeID: 13)
      💬 Args: [AIntialBalance - boldToken.balanceOf(A), (troveInitialDebt + troveAccruedInterest) + troveAccruedManagementFee]
      👁️  Def: internal
```
