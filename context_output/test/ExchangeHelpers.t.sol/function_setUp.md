...
function setUp() external {
    ...
    exchangeHelpersV2 = new HybridCurveUniV3ExchangeHelpersV2({_usdc: USDC, _weth: WETH, _curvePool: ICurveStableswapNGPool(address(curveUsdcBold)), _usdcIndex: int8((curveUsdcBold.coins(0) == USDC) ? 0 : 1), _boldIndex: int8((curveUsdcBold.coins(0) == BOLD) ? 0 : 1), _feeUsdcWeth: UNIV3_FEE_USDC_WETH, _feeWethColl: UNIV3_FEE_WETH_COLL, _uniV3Quoter: uniV3Quoter});
}
```
...
```
┌─ [0] ⚙️ FUNCTION: ExchangeHelpersTest.setUp() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
